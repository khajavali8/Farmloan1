...
function Transactions() {
    ...
    return (
        <div style={{ padding: "20px", fontFamily: "'Segoe UI', Tahoma, Geneva, Verdana, sans-serif", maxWidth: "950px", margin: "0 auto" }}>
            <h2 style={{ textAlign: "center", color: "#2e7d32", marginBottom: "20px", fontSize: "26px", fontWeight: "600" }}>My Transactions</h2>
            <table style={{ width: "100%", borderCollapse: "collapse", backgroundColor: "#fff", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)" }}>
                <thead>
                ...
                <tbody>
                    {currentTransactions.map((tx) => (
                        <tr key={tx._id} style={{ transition: "background 0.2s ease", backgroundColor: tx.type === "credit" ? "#e8f5e9" : "#ffebee" }}>
                            <td style={{ padding: "12px", border: "1px solid #ddd" }}>{tx._id}</td>
                            <td style={{ padding: "12px", border: "1px solid #ddd", color: tx.type === "credit" ? "#2e7d32" : "#c62828", fontWeight: "500" }}>
                                {typeIcon(tx.type)} {tx.type.charAt(0).toUpperCase() + tx.type.slice(1)}
                            </td>
                            <td style={{ padding: "12px", border: "1px solid #ddd" }}>{tx.amount}</td>
                            <td style={{ padding: "12px", border: "1px solid #ddd" }}>{new Date(tx.createdAt).toLocaleString()}</td>
                            <td style={{ padding: "12px", border: "1px solid #ddd" }}>
                ...
                {Array.from({ length: totalPages }, (_, i) => (
                    <button
                        ...
                        style={{
                            margin: "0 5px",
                            padding: "6px 12px",
                            backgroundColor: currentPage === i + 1 ? "#2e7d32" : "#f1f1f1",
            ...
            {selectedTransaction && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <h3 style={{ color: "#2e7d32", marginBottom: "15px", fontSize: "20px", fontWeight: "600" }}>Transaction Details</h3>
                        <p><strong>Type:</strong> {selectedTransaction.type}</p>
                        <p><strong>Amount:</strong> {selectedTransaction.amount}</p>
                        <p><strong>Sent At:</strong> {new Date(selectedTransaction.createdAt).toLocaleString()}</p>

                        <h3 style={{ color: "#2e7d32", marginTop: "20px", marginBottom: "15px", fontSize: "18px", fontWeight: "600" }}>Sender Details</h3>
                        <p><strong>Name:</strong> {selectedTransaction.from ? `${selectedTransaction.from.firstName} ${selectedTransaction.from.lastName}` : "N/A"}</p>
                        <p><strong>Email:</strong> {selectedTransaction.from?.email || "N/A"}</p>

                        <h3 style={{ color: "#2e7d32", marginTop: "20px", marginBottom: "15px", fontSize: "18px", fontWeight: "600" }}>Receiver Details</h3>
                        <p><strong>Name:</strong> {selectedTransaction.to ? `${selectedTransaction.to.firstName} ${selectedTransaction.to.lastName}` : "N/A"}</p>
                        <p><strong>Email:</strong> {selectedTransaction.to?.email || "N/A"}</p>

                        <button onClick={closeModal} style={{
                            marginTop: "15px", padding: "8px 12px", background: "#c62828",
                            color: "white", border: "none", borderRadius: "5px", cursor: "pointer"
                        }}>Close</button>
                    </div>
                </div>
